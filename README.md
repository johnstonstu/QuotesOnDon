...
| `tools/` | `ingest.mjs`, `dashboard.mjs`, `validate-quotes.mjs`, `migrate-legacy-seed.mjs`. |
...
| --- | --- | --- | --- |
| 1 | **X — @realDonaldTrump** | **off, needs `X_BEARER_TOKEN`** | His own words. X's free tier cannot read another account's posts; reading him needs the paid Basic tier. The adapter is written and waits on a token. |
| 1b | **Truth Social** (`trumpstruth.org/feed`) | **on, working** | His own posts, full text, a stable URL per post, no auth. Same words as the X feed in practice, since he cross-posts. |
...
`~/.hermes/.env`; nothing is committed.
